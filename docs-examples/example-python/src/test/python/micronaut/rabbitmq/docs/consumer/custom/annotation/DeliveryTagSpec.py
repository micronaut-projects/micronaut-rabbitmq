...
class DeliveryTagSpec:
    ...
    @Test
    def test_using_a_custom_annotation_binder(self) -> None:
# tag::producer[]
        self.product_client.send(b"body")
        self.product_client.send(b"body2")
        self.product_client.send(b"body3")
# end::producer[]

        Await.until(lambda: len(self.product_listener.messages) == 3)
