...
[.lang-python]
****
A Python class cannot extend the `ChannelInitializer` Java class, so the Python example implements the api:configuration.rabbitmq.connect.ChannelPoolInitializer[] interface instead.
****
