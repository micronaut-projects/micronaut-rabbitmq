...
if [ "${TRAVIS_JDK_VERSION}" == "openjdk11" ] ; then
    ...
    ./gradlew --stop
    ./gradlew check --no-daemon --fail-fast || EXIT_STATUS=$?
    ...
    if [[ -n $TRAVIS_TAG ]] || [[ $TRAVIS_BRANCH =~ ^master$ && $TRAVIS_PULL_REQUEST == 'false' ]]; then
      ...
      if [[ $EXIT_STATUS -eq 0 ]]; then
       ./gradlew --console=plain --no-daemon docs  || EXIT_STATUS=$?
        ...
        git clone https://${GH_TOKEN}@github.com/micronaut-projects/micronaut-rabbitmq.git -b gh-pages gh-pages --single-branch > /dev/null
